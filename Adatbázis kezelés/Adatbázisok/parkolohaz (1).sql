...
-- Eljárások
--
DROP PROCEDURE IF EXISTS `datummuvelet`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `datummuvelet` (IN `dt2` DATETIME, IN `dt1` DATETIME)   BEGIN
    ...
    SET ora1 = HOUR(dt1);
    SET ora2 = HOUR(dt2);
    SET perc1 = MINUTE(dt1);
    SET perc2 = MINUTE(dt2);
    #SELECT ev1,ev2,ho1,ho2,nap1,nap2,ora1,ora2,perc1,perc2;
	#SELECT TIMESTAMPDIFF(HOUR,dt1,dt2);
    SELECT ADDTIME(dt1,-5);
END$$
...
DROP TABLE IF EXISTS `parkolasok`;
...
DELIMITER $$
CREATE TRIGGER `szamlazas` AFTER UPDATE ON `parkolasok` FOR EACH ROW BEGIN
	DECLARE diff INT;
    DECLARE osszeg INT;
    SET diff = TIMESTAMPDIFF(MINUTE, OLD.kezdete, NEW.vege);
    SET osszeg = diff*10;
    INSERT INTO szamlak(kelt,rendszam,osszeg) VALUES(NOW(), OLD.rendszam,osszeg);
END
$$
DELIMITER ;
...
--
ALTER TABLE `szamlak`
  ADD PRIMARY KEY (`szlaszam`),
  ADD KEY `FKszamla` (`rendszam`);
...
--
ALTER TABLE `parkolasok`
  ADD CONSTRAINT `parkolasok_ibfk_1` FOREIGN KEY (`rendszam`) REFERENCES `autok` (`rendszam`);

--
-- Megkötések a táblához `szamlak`
--
ALTER TABLE `szamlak`
  ADD CONSTRAINT `szamlak_ibfk_1` FOREIGN KEY (`rendszam`) REFERENCES `autok` (`rendszam`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
